...
class FosterUsers(models.Model):
    ...
    def search(self, args, offset=0, limit=0, order=None, count=False):
        if 'user_type' in self._context:
            ...
            if type == 'admin':
                args = [('groups_id', '=', self.env.ref("foster-base.group_foster_admin").id)]
            elif type == 'manager':
                args = [('groups_id', '=', self.env.ref("foster-base.group_foster_manager").id)]
            elif type == 'supervisor':
                args = [('groups_id', '=', self.env.ref("foster-base.group_foster_supervisor").id)]
            elif type == 'case_worker':
                args = [('groups_id', '=', self.env.ref("foster-base.group_case_worker").id)]
            elif type == 'family_resource':
                args = [('groups_id', '=', self.env.ref("foster-base.group_foster_family_resource").id)]
    ...
    def create(self, vals):
        if self.env.user.has_group('foster-base.group_foster_admin'):
            self = self.sudo()
        ...
        if 'mp_manager_menu' in self._context:
            group_id = self.env.ref('foster-base.group_foster_manager').id
            vals['groups_id'] = [group_id]
        if 'mp_admin_menu' in self._context:
            group_id = self.env.ref('foster-base.group_foster_admin').id
            vals['groups_id'] = [group_id]
        if 'mp_supervisor_menu' in self._context:
            group_id = self.env.ref('foster-base.group_foster_supervisor').id
            vals['groups_id'] = [group_id]
        if 'mp_family_resource_menu' in self._context:
            group_id = self.env.ref('foster-base.group_foster_family_resource').id
            vals['groups_id'] = [group_id]
        if 'mp_case_worker_menu' in self._context:
            group_id = self.env.ref('foster-base.group_case_worker').id
            vals['groups_id'] = [group_id]
    ...
    @api.multi
    def write(self, vals):
        is_admin = self.env.user.has_group('foster-base.group_foster_admin') or self.env.user.id == 1
        if is_admin:
            self = self.sudo()
