...
var CrashManager = core.Class.extend({
    ...
    rpc_error: function(error) {
        console.log("Some rpc error", error);
        return;
        var self = this;
        if (!this.active) {
            return;
        ...
        if (error.data.name === "odoo.http.SessionExpiredException" || error.data.name === "werkzeug.exceptions.Forbidden") {
            ...
            //alerd(handled in ajax.js
            dntoast.error("Please <a href='/web/login'><b>Click Here</b></a> to reload session.", 500);
            return;
        }
    ...
    show_message: function(exception) {
        console.log("Some message", exception);
        //dntoast.error("Please <a href='/web/login'><b>Click Here</b></a> to reload session..", 500);
//        this.show_error({
