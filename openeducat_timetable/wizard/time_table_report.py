...
class SessionReport(models.TransientModel):
    ...
    state = fields.Selection(
        [('faculty', 'Faculty'), ('student', 'Class')],
        string='Select', required=True, default='student')
    ...
    def gen_time_table_report(self):
        ...
        if data['state'] == 'student':
            ...
            data.update({'faculty': self.batch_id.faculty_id.name})
            timing = str(timedelta(hours=self.batch_id.start)).rsplit(':', 1)[0] + '-' + \
                    str(timedelta(hours=self.batch_id.end)).rsplit(':', 1)[0]
            ...
            data.update({'faculty': self.faculty_id.name})
