                ...
                for line in session.time_table_lines:
                    if int(line.day) == curr_date.weekday():
                        ...
                        if available:
                            ...
                            raise ValidationError(_("%s not available at selected time %s to %s." % (line.faculty_id.name,final_date.strftime("%Y-%m-%d %H:%M:%S"),t.strftime("%Y-%m-%d %H:%M:%S"))))
...
class GenerateSessionLine(models.TransientModel):
    ...
    def _compute_availaible(self):
        for record in self:
            str = ""
            for a in record.faculty_id.available_times:
