...
class OpSession(models.Model):
    ...
    timing_id = fields.Many2one(
        'op.timing', 'Timing', track_visibility="onchange")
    ...
    def _compute_availaible(self):
        for record in self:
            str=""
            for a in record.availaible_times:
    ...
    def _compute_name(self):
        for session in self:
            if session.faculty_id and session.batch_id:
                session.name = session.faculty_id.name + ':' + \
                    session.course_id.name + ':' + str(datetime.timedelta(hours=session.batch_id.start)).rsplit(':', 1)[0] + '-' +\
                               str(datetime.timedelta(hours=session.batch_id.end)).rsplit(':', 1)[0]
