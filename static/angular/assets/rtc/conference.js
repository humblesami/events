(function(){
    ...
    function init_video_call(){
        ...
        document.getElementById('leave-room').onclick = function(){
            if(connection)
            {
                $('#rtc-container').html(original_rtc_html);
                init_video_call();
            }
        };
    ...
    var timeout_waiter = 0;
    function wait_rtc_lib()
    {       
        console.log(3223, timeout_waiter);
        timeout_waiter++; 
        try{
            if(RTCMultiConnection)
            {
                init_video_call();
            }
            else{
