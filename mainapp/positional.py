...
class PositionalSortMixIn(object):
    ...
    def get_object_at_offset(self, offset):
        ...
        try:
            return model_class._default_manager.get(position=self.position + offset)
    ...
    def move_down(self):
        ...
        final = model_class._default_manager.all().order_by('-position')[0].position + 1
    ...
    def insert_at(self, position):
        """
            Saves the object at a specified position. The `position` field
        ...
        objects_after = model_class._default_manager.filter(position__gte=position).order_by('-position')
    ...
    def swap_position(self, other):
        """
            ...
            the
            position
            with some other class instance
        """
    ...
    def save(self):
        model_class = self.__class__
        # is there a position saved? (explicitly testing None because 0 would befalse as well)
    ...
    def delete(self):
        ...
        objects_after = model_class._default_manager.filter(position__gt=self.position)
