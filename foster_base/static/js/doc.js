...
console.log('helo');
    ...
    setTimeout(function(){
        ...
        if(seen != 1)
        {
            ...
            if(res_id)
            {
                update_seen_by('foster.sign_docs', res_id);
            }
        }
    },900);
//    $('.doc_view_form').closest('.modal-body').css('overflow-y','hidden');
    if ($('.my_signature_status').text()=="Not required"){
        $('#start_sign').hide();
    }
    if ($('.my_signature_status').text()=="Completed"){
        $('#start_sign').hide();
    }
